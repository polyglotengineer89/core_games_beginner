--[[
	Modified Based on NPCAI - Server
	...
	Modified by randomphantom
	- able to move around in air, subject to min and max z and collisions.
	
	
--]]

local MAX_Z = script:GetCustomProperty("MaxZ")
local MIN_Z = script:GetCustomProperty("MinZ")
local GROUND_BUFFER = 100 --needed to prevent case where it lands exactly on the ground, and a raycast from that position doesn't detect any ground.
...
-------------------------------------------------------------------------------------------------------------CHECK FOR HITTING MAX Z
	local intersects = rayStart

	local impactPos = nil

	if direction.z == 0 then
		--moving on same Z plane
		local hitResult = World.Raycast(pos, rayStart, {ignorePlayers = true})
		if hitResult then
			impactPos = hitResult:GetImpactPosition()
		end
	else
		--changing Z positions - check for Z plane collision first. if so, limit to max Z
		if rayStart.z > MAX_Z then 
			rayStart.z = MAX_Z 
			direction = rayStart - pos
			intersects = rayStart
		end
		--check for collisions with ground, based on the Z-constrained movement.
		local hitResult = World.Raycast(pos, rayStart, {ignorePlayers = true})
		if hitResult then
			--something blocking in a direct raycast. 
			--attempt two ways: moving on Z plane as player, or moving on the same Z plane as shark
			--in general as ground is below, test moving on Z plane as player first as ground edge may block.
			local hitResult2 = World.Raycast(pos + Vector3.New(0, 0, direction.z), rayStart, {ignorePlayers = true})
			if hitResult2 then
				local hitResult3 = World.Raycast(pos, pos + Vector3.New(direction.x, direction.y,0), {ignorePlayers = true})
				if hitResult3 then
					--totally blocked. stop moving at the original hitResult 
					impactPos = hitResult:GetImpactPosition()					
				else
					--move on same z plane.
					intersects = pos + Vector3.New(direction.x, direction.y,0)
				end
			else
				--move up or down.
				intersects = pos + Vector3.New(0, 0, direction.z)
			end
		end
	end
	if impactPos then
		if (impactPos - pos).sizeSquared < GROUND_BUFFER * GROUND_BUFFER then
			intersects = pos
		else
			intersects = impactPos - GROUND_BUFFER * (direction:GetNormalized())
		end					
	end
	
	stepDestination = intersects	
